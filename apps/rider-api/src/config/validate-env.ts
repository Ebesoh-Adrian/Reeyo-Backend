...
interface EnvironmentVariables {
  ...
  // Rider Configuration
  MIN_EARNINGS_FOR_PAYOUT: number;
...
export function validateEnv(): EnvironmentVariables {
  ...
  if (missingVars.length > 0) {
    throw new Error(
      `Missing required environment variables: ${missingVars.join(', ')}\n` +
        `Please check your .env file in apps/rider-api/`
    );
  }
  ...
  return {
    ...
    LOG_FILE_PATH: process.env.LOG_FILE_PATH || './logs/rider-api.log',
    ...
    // Rider Configuration
    MIN_EARNINGS_FOR_PAYOUT: parseInt(process.env.MIN_EARNINGS_FOR_PAYOUT!, 10) || 5000,
    PAYOUT_PROCESSING_FEE: parseInt(process.env.PAYOUT_PROCESSING_FEE!, 10) || 200,
    DEFAULT_DELIVERY_RADIUS_KM: parseInt(process.env.DEFAULT_DELIVERY_RADIUS_KM!, 10) || 10,
  };
}
